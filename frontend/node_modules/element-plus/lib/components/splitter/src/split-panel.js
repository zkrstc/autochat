...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { emit: emits }) {
    ...
    const { panels, layout, containerSize, pxSizes } = vue.toRefs(splitterContext);
    const {
      registerPanel,
      sortPanel,
      unregisterPanel,
    ...
    const _panel = vue.reactive({
      ...
      setIndex,
      ...props,
      collapsible: usePanel.getCollapsible(props.collapsible)
    });
    registerPanel(_panel);
    vue.onMounted(() => {
      sortPanel(_panel);
    });
    vue.onUnmounted(() => unregisterPanel == null ? void 0 : unregisterPanel(uid));
    return (_ctx, _cache) => {
